...
(defun gilbert/fix-postgres-prompt-regexp ()
...
(add-hook 'sql-interactive-mode-hook 'gilbert/fix-postgres-prompt-regexp)

(defun gilbert/pop-to-sqli-buffer ()
  ...
  (if (and sql-buffer (buffer-live-p sql-buffer))
    ...
    (sql-set-sqli-buffer)
    (when sql-buffer
      (gilbert/pop-to-sqli-buffer))))

(with-eval-after-load 'sql
  (define-key sql-mode-map (kbd "C-c C-z") 'gilbert/pop-to-sqli-buffer)
  (when (package-installed-p 'dash-at-point)
    (defun gilbert/maybe-set-dash-db-docset (&rest _)
      (when (eq sql-product 'postgres)
        (setq-local dash-at-point-docset "psql")))

    (add-hook 'sql-mode-hook 'gilbert/maybe-set-dash-db-docset)
    (add-hook 'sql-interactive-mode-hook 'gilbert/maybe-set-dash-db-docset)
    (advice-add 'sql-set-product :after 'gilbert/maybe-set-dash-db-docset)))
...
(defun gilbert/font-lock-everything-in-sql-interactive-mode ()
  (unless (eq 'oracle sql-product)
    (sql-product-font-lock nil nil)))
(add-hook 'sql-interactive-mode-hook 'gilbert/font-lock-everything-in-sql-interactive-mode)
...
;; Package ideas:
;;   - PEV
(defun gilbert/sql-explain-region-as-json (beg end &optional copy)
